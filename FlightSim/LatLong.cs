...
namespace FlightSim
{
    public class Latitude
    {
        ...
        public virtual string ToString(bool secondsHundreth)
        {
            if (secondsHundreth)
            {
                return string.Format("{0}° {1}' {2:0.00}\" {3}", Math.Abs(Degrees), Minutes, Seconds, Degrees >= 0 ? "N" : "S");
            }
            return string.Format("{0}° {1}' {2:0}\" {3}", Math.Abs(Degrees), Minutes, Seconds, Degrees >= 0 ? "N" : "S");
        }

        public virtual string ToShortString()
        {
            return string.Format("{0}° {1}' {2}", Math.Abs(Degrees), Minutes, Degrees >= 0 ? "N" : "S");
        }
    }
    ...
    public class Longitude : Latitude
    {
        ...
        public override string ToString(bool secondsHundreth)
        {
            if (secondsHundreth)
            {
                return string.Format("{0}° {1}' {2:0.00}\" {3}", Math.Abs(Degrees), Minutes, Seconds, Degrees >= 0 ? "E" : "W");
            }
            return string.Format("{0}° {1}' {2:0}\" {3}", Math.Abs(Degrees), Minutes, Seconds, Degrees >= 0 ? "E" : "W");
        }

        public override string ToShortString()
        {
            return string.Format("{0}° {1}' {2}", Math.Abs(Degrees), Minutes, Degrees >= 0 ? "E" : "W");
        }
    }
