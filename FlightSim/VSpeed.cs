    ...
    public class VSpeed
    {
        ...
        public static VSpeed GetVSpeed(int aircraftId)
        {
            try
            {
                ...
                {
                    VSpeed? vSpeed = JsonConvert.DeserializeObject<VSpeed>(response.Content);
                    return vSpeed != null ? vSpeed : null!;
                }
            }
            catch(Exception)
            {
        ...
        public static List<VSpeed> GetAllVSpeeds()
        {
            try
            {
                ...
                if (response != null && response.IsSuccessful && !string.IsNullOrEmpty(response.Content))
                {
                    List<VSpeed>? vSpeeds = JsonConvert.DeserializeObject<List<VSpeed>>(response.Content);
                    return vSpeeds != null ? vSpeeds : new List<VSpeed>();
                }
            }
            catch(Exception)
            {
