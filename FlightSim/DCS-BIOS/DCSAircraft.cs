namespace FlightSim.DCS_BIOS
{
    ...
    public class DCSAircraft
    {
        ...
        public static DCSAircraft SelectedAircraft { get; set; } = null!;
        ...
        public static void FillModulesListFromDcsBios(string dcsbiosJsonFolder, bool loadInternalModules)
        {
            ...
            {
                var resourceName = "FlightSim.dcs-bios_modules.txt";
                var asm = Assembly.GetExecutingAssembly();
                using Stream? s = asm.GetManifestResourceStream(resourceName);
                if (s == null)
                    throw new FileNotFoundException($"Embedded resource not found: {resourceName}");
                using var fs = new FileStream(dcsbiosConfigFile, FileMode.Create, FileAccess.Write);
                s.CopyTo(fs);
            }
            if (!File.Exists(dcsbiosConfigFile))
            {
        ...
        public static DCSAircraft GetBackwardCompatible(string oldEnumValue)
        {
            ...
            {
                DCSAircraft? aircraft = Modules.Find(o => o.ID == moduleNumber);
                if (aircraft != null)
                {
                    return aircraft;
                }
