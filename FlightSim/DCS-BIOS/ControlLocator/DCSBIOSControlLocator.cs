...
using FlightSim.DCS_BIOS.StringClasses;


namespace FlightSim.DCS_BIOS.ControlLocator
{
    using System;
    ...
    public static class DCSBIOSControlLocator
    {
        ...
        private static DCSAircraft _dcsAircraft = null!;
        private static string _jsonDirectory = null!;
        ...
        private static void LoadControls()
        {
            ...
            try
            {
                ...
                {
                    var ns430 = DCSAircraft.GetNS430();
                    if (ns430 != null)
                    {
                        if (DCSBIOSAircraftLoadStatus.IsLoaded(ns430.JSONFilename))
                        {
                        ...
                        {
                            DCSBIOSAircraftLoadStatus.Remove(ns430.JSONFilename);
                        }
                    }
                }
                ...
                if (Common.IsEmulationModesFlagSet(EmulationMode.NS430Enabled))
                {
                    ...
                    if (ns430 != null)
                    {
                        ReadDataFromJsonFile(ns430.JSONFilename);
                    }
                }
        ...
        {
            var keyEmulator = DCSAircraft.GetKeyEmulator();
            var noFrame = DCSAircraft.GetNoFrameLoadedYet();

            if (DCSBIOSAircraftLoadStatus.IsLoaded(filename) ||
                (keyEmulator != null && filename == keyEmulator.JSONFilename) ||
                (noFrame != null && filename == noFrame.JSONFilename))
            {
                return;
            }
