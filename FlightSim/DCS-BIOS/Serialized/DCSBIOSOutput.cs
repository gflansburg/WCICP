...
namespace FlightSim.DCS_BIOS.Serialized
{
    using System;
    ...
    public class DCSBIOSOutput
    {
        ...
        public void Consume(DCSBIOSControl dcsbiosControl, DCSBiosOutputType dcsBiosOutputType)
        {
            ...
            try
            {
                ...
                foreach (var input in dcsbiosControl.Inputs.Where(i => i?.SuggestedStep is >= 0))
                {
                    _defaultVariableChangeValue = input!.SuggestedStep!.ToString() ?? string.Empty;
                }
        ...
        public bool StringValueHasChanged(ushort address, string? stringData)
        {
            ...
            lock (_lockObject)
            {
                ...
                var normalized = stringData ?? string.Empty;

                if ((_lastStringValue ?? string.Empty) != normalized)
                {
                    _lastStringValue = normalized;
                    return true;
                }
