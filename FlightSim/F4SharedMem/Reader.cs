...
using FlightSim.F4SharedMem.Win32;
using System;
    ...
    public sealed class Reader : IDisposable
    {
        ...
        public FlightData GetCurrentData()
        {
           var dataType = typeof(BMS4FlightData);
            ...
            if (data is BMS4FlightData flightData)
            {
                var toReturn = new FlightData(flightData);

                if (!_hSecondarySharedMemoryAreaFileMappingObject.Equals(IntPtr.Zero))
                {
                    var data2 = (Marshal.PtrToStructure(_lpSecondarySharedMemoryAreaBaseAddress, typeof(FlightData2)));
                    if (data2 is FlightData2 flightData2)
                        toReturn.PopulateFromStruct(data2);
                }

                if (!_hOsbSharedMemoryAreaFileMappingObject.Equals(IntPtr.Zero))
                {
                    var data2 = (Marshal.PtrToStructure(_lpOsbSharedMemoryAreaBaseAddress, typeof(OSBData)));
                    if (data2 is OSBData obsData)
                        toReturn.PopulateFromStruct(obsData);
                }

                if (!_hIntelliVibeSharedMemoryAreaFileMappingObject.Equals(IntPtr.Zero))
                {
                    var data2 = (Marshal.PtrToStructure(_lpIntelliVibeSharedMemoryAreaBaseAddress, typeof(IntellivibeData)));
                    if (data2 is IntellivibeData intellivibeData)
                        toReturn.IntellivibeData = intellivibeData;
                }

                if (!_hRadioClientControlSharedMemoryAreaFileMappingObject.Equals(IntPtr.Zero))
                {
                    var data2 = (Marshal.PtrToStructure(_lpRadioClientControlSharedMemoryAreaBaseAddress, typeof(RadioClientControl)));
                    if (data2 is RadioClientControl radioClientControlData)
                        toReturn.RadioClientControlData = radioClientControlData;
                }

                if (!_hRadioClientStatusSharedMemoryAreaFileMappingObject.Equals(IntPtr.Zero))
                {
                    var data2 = (Marshal.PtrToStructure(_lpRadioClientStatusSharedMemoryAreaBaseAddress, typeof(RadioClientStatus)));
                    if (data2 is RadioClientStatus statusData)
                        toReturn.RadioClientStatus = statusData;
                }
                ...
                if (!_hStringSharedMemoryAreaFileMappingObject.Equals(IntPtr.Zero))
                {
                    ...
                    toReturn.StringData = StringData.GetStringData(rawStringData ?? new byte[0]);
                }
                ...
                if (!_hDrawingSharedMemoryAreaFileMappingObject.Equals(IntPtr.Zero))
                {
                    ...
                    toReturn.DrawingData = DrawingData.GetDrawingData(rawDrawingData ?? new byte[0]);
                }
