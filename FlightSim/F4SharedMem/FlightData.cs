...
namespace FlightSim.F4SharedMem
{
    [Serializable]
    public sealed class FlightData
    {
        ...
        internal void PopulateFromStruct<T>(T data)
        {
            if (data != null)
            {
                bool foundRelevantHeaderFields = _headerFields.TryGetValue(data.GetType(), out FieldInfo[]? relevantHeaderFields);
                ...
                if (relevantHeaderFields != null)
                {
                    foreach (var currentField in relevantHeaderFields)
                    {
                        ...
                        {
                            bool fieldInfoFoundInCache = _flightDataFields.TryGetValue(currentField.Name, out FieldInfo? thisFlightDataField);
                    ...
                    else if (currentFieldType == typeof(EWPI_LineOfText[]))
                    {
                        PopulateEWPILineOfText(data, currentField, thisFlightDataField);
                                ...
                                {
                                    var value = currentField.GetValue(data);
                                    if (value is Array currentValue)
                                    {
        ...
        {
            var value = currentField.GetValue(data);
            if (value is DED_PFL_LineOfText[] currentValue)
            {
        ...
        {
            var value = currentField.GetValue(data);
            if (value is Callsign_LineOfText[] currentValue)
            {
                var valuesToAssign = new string[currentValue.Length];
                for (var j = 0; j < currentValue.Length; j++)
                {
        ...
        {
            var value = currentField.GetValue(data);
            if (value is EWMU_LineOfText[] currentValue)
            {
                var valuesToAssign = new string[currentValue.Length];
                for (var j = 0; j < currentValue.Length; j++)
                {
        ...
        {
            var value = currentField.GetValue(data);
            if (value is EWPI_LineOfText[] currentValue)
            {
                var valuesToAssign = new string[currentValue.Length];
                for (var j = 0; j < currentValue.Length; j++)
                {
        ...
        {
            var value = currentField.GetValue(data);
            if (value is OSBLabel[] currentValue)
            {
        ...
        //DED Lines
        public string[] DEDLines = null!;  //25 usable chars
        public string[] Invert = null!;    //25 usable chars

        //PFL Lines
        public string[] PFLLines = null!;  //25 usable chars
        public string[] PFLInvert = null!; //25 usable chars
        ...
        public int[] RWRsymbol = null!;
        public float[] bearing = null!;
        public uint[] missileActivity = null!;
        public uint[] missileLaunch = null!;
        public uint[] selected = null!;
        public float[] lethality = null!;
        public uint[] newDetection = null!;
        ...
        public byte[] tacanInfo = null!;    //TACAN info (new in BMS4)
        public float fuelFlow2;     // Ownship fuel flow2 (Lbs/Hour)
        public byte[] RwrInfo = null!;      //[512] New RWR Info
        ...
        public string[] pilotsCallsign = null!;   // [MAX_CALLSIGNS][CALLSIGN_LEN] List of pilots callsign connected to an MP session
        public byte[] pilotsStatus = null!;     // [MAX_CALLSIGNS] Status of the MP pilots, see enum FlyStates
        ...
        //VERSION 12
        public ushort[] RTT_size = null!;  //[2]                    RTT overall width and height
        public ushort[] RTT_area = null!;  // [RTT_noOfAreas][4]    For each area: left/top/right/bottom
        ...
        public uint[] ecmBits = null!; // see EcmBits enum for details - Note: these are currently not combinable bits, but mutually exclusive states!

        public EcmOperStates ecmOper;                  // (unsigned char) see enum EcmOperStates for details

        public JammingStates[] RWRjammingStatus = null!; // (unsigned) char see enum JammingStates for details
...
#endif
        public OptionSelectButtonLabel[] leftMFD = null!;
        public OptionSelectButtonLabel[] rightMFD = null!;
        public object ExtensionData = null!;
        ...
        public StringData StringData { get; set; } = null!;
        public DrawingData DrawingData { get; set; } = null!;
    }
}